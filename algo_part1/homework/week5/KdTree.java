...
public class KdTree {
    ...
    private Node put(Node node, Point2D point, boolean isVertical, Node prev){
        if (node == null){
            size++;
    ...
    private Node get(Node node, Point2D point){
        ...
        if (node.compareTo(point) > 0) return get(node.left, point);
        else return get(node.right, point);
    }
    ...
    private boolean updateNearestPointIfNeeded(Point2D targetPoint, Point2D newPoint) {
        ...
        if (distanceToNearest > distanceToNew){
            nearestPoint = newPoint;
            return true;
        }
